<p align="center"><img src="https://github.com/aaskjer/Better-Stream-Announcements/blob/main/Assets/Bsa-Full.png?raw=true" alt="BsA banner" width="400" height="500"></p>

Manage livestream announcements in Discord for multiple platforms.
Post embed stream notifications, supports streamer friends and community promotions.
*Inspired by streamcord*

![Static Badge](https://img.shields.io/badge/https%3A%2F%2Fimg.shields.io%2Fbadge%2Fany_text-Twitch%2C_Kick%2C_YouTube-white?label=supported%20plattform)
![Static Badge](https://img.shields.io/badge/https%3A%2F%2Fimg.shields.io%2Fbadge%2Fany_text-streamer.bot-white?label=tool&color=%230b73ff)
![GitHub License](https://img.shields.io/github/license/aaskjer/Better-Stream-Announcements)
![GitHub Release](https://img.shields.io/github/v/release/aaskjer/Better-Stream-Announcements)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/aaskjer/Better-Stream-Announcements/total)

---

# Overview

- **Automatic Discord announcements**  
  Posts embed go-live and stream-ended messages for your own channel directly from Streamer.bot.

- **Multi-platform support**  
  Works with Twitch, YouTube, Kick, and Trovo, so your announcements stay consistent across platforms.

- **Customizable embed content**  
  Add game info, viewer counts, follower counts, tags, box art, preview images, and a Watch button to match your style.

- **Streamer Friends support**  
  Tracks selected creators and can announce their streams too, with mappings for Discord roles and platform accounts.

- **Flexible Discord controls**  
  Supports @everyone, @here, role mentions, auto-crossposting, and configurable cooldowns for cleaner server management.

- **Self-service Discord setup**  
  Includes a Discord bot presence and self-assign flow so users can register themselves for announcements and roles.

- **Built-in settings GUI**  
  Comes with a GUI window for quick configuration, theming, and troubleshooting without editing code.

Note: This project also work great running 24/7 on a remote server


---

## Get Started

[Setup Guide](https://github.com/aaskjer/Better-Stream-Announcements/blob/main/Setup-Guide.md)

[Frequently Asked Questions](https://github.com/aaskjer/Better-Stream-Announcements/blob/main/FAQ.md)

[Import String for streamer.bot](https://github.com/aaskjer/Better-Stream-Announcements/blob/main/Import-String.md)

[Download](https://github.com/aaskjer/Better-Stream-Announcements/releases)

---

## Credits & Links


[aaskjer on Twitch](https://twitch.tv/aaskjer)

[Better Stream Announcements on SB Discord](https://discord.com/channels/834650675224248362/1449362980524396645/1449362980524396645)

[Streamer.bot](https://streamer.bot) / [nate1280](https://www.patreon.com/c/nate1280/home)

[Sorylokan](https://extensions.streamer.bot/u/Sorylokan/summary)

YOU ♡
